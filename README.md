...
- **타일 시스템 구조화 경험**
처음부터 공통 기능을 관리할 수 있도록 Tile이라는 부모 클래스를 정의하고, GroundTile, ResourceTile 등 자식 클래스로 분리해 구조적으로 설계하였습니다.
맵 배치는 TileGridManager를 통해 처리하되, 각 타일의 머티리얼과 타입은 구조체로 통합 관리하고자 TileDataAsset 클래스를 추가로 설계하였습니다.
이를 통해 언리얼 에디터에서 데이터 기반으로 시각적으로 타일 속성을 관리할 수 있었고, 타일 확장 및 유지보수 시 코드 수정 없이 데이터 변경만으로도 대응이 가능해졌습니다.

- **컴포넌트 기반 리팩토링 경험**  
채굴 기능이 캐릭터, 타일, UI 등 다양한 요소와 연결되어 있어 유지보수가 어려웠습니다. 이를 MiningComponent로 분리함으로써 기능의 경계를 명확히 하고, HUD 연동 작업이나 애니메이션 수정 시에도 해당 컴포넌트만 집중적으로 관리할 수 있어 구조적인 이해와 작업 흐름이 쉬워졌습니다.
- **코드 흐름의 중요성 체감**  
여러 기능이 한 클래스에 뒤섞여 있을 때 디버깅이나 추가 구현이 어려웠습니다. 각 기능을 역할 단위로 나누고 흐름을 정리하자, 타일 배치나 자원 채굴 시 문제가 생겼을 때 빠르게 원인을 파악하고 수정할 수 있었고, 전체 코드에 대한 이해도도 높아졌습니다.

- **협업을 위한 코드 작성 습관 개선**  
Sourcetree를 통한 버전 관리와 함께, Push 시 작업 내용을 팀원에게 요약 보고하는 규칙을 운영했습니다. 또한, 함수 단위 주석과 의미 있는 변수명 작성에 집중하여, 다른 팀원이 제 코드를 읽고 쉽게 이해하고 이어서 작업할 수 있도록 신경 썼습니다.

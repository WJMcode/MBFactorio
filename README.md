...
- **다중 자원 타일 오버랩 인식 문제**
  - **문제 배경**  
    플레이어가 두 개 이상의 자원 타일(예: 구리, 철)과 동시에 오버랩할 경우, 처음 감지된 자원만 인식되고 나머지는 무시되는 현상이 발생하였습니다.
  - **원인**  
    하나의 자원 타일만 인식하도록 구현되어 있었기 때문입니다.
  - **해결 과정**  
    `FindOverlappingTile` 함수를 도입하여 오버랩 해제 시, 주변 자원 타일을 재탐색하고 가장 가까운 타일을 다시 인식하도록 구조를 개선하였습니다.
  - **결과**  
    여러 자원 타일과 동시에 오버랩해도 가장 가까운 타일을 정상적으로 인식할 수 있게 되었습니다.
...
| 구현 요소       | 설명 |
|----------------|------|
| 타일 랜덤 배치 | 플레이어 주변에 확률 기반으로 다양한 타일 생성 |
| 타입/머티리얼 자동 할당 | 타입별 머티리얼 세트를 구조체 기반으로 관리, Data Asset과 연동 |

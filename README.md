...
## ⚠️ 트러블슈팅
- **다중 자원 타일 오버랩 인식 문제**
  - **문제 배경** : 플레이어가 두 개 이상의 자원 타일(예: 구리, 철)과 동시에 오버랩할 경우, 처음 감지된 자원만 인식되고  
    나머지는 무시되는 현상이 발생하였습니다.
  - **원인** : 하나의 자원 타일만 인식하도록 구현되어 있었기 때문입니다.
  - **해결 과정** : `FindOverlappingTile` 함수를 도입하여 오버랩 해제 시, 주변 자원 타일을 재탐색하고  
     가장 가까운 타일을 다시 인식하도록 구조를 개선하였습니다.

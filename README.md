...
- **타일 시스템 구조화 경험**
공통 기능을 관리할 수 있도록 Tile이라는 부모 클래스를 정의하고 GroundTile, ResourceTile 등 자식 클래스로 분리해 구조적으로 설계하였습니다.
타일 배치는 TileGridManager를 통해 처리하되, 각 타일의 머티리얼과 타입은 구조체로 통합 관리하고자 TileDataAsset 클래스를 추가로 설계하였습니다.
이를 통해 언리얼 에디터에서 데이터 기반으로 타일 속성을 관리할 수 있었고 타일 확장 및 유지보수 시 코드 수정 없이 데이터 변경만으로도 대응이 가능해졌습니다.

- **컴포넌트 기반 리팩토링 경험**  
채굴 기능이 캐릭터, 타일, UI 등 다양한 요소와 연결되어 있어 유지보수가 어려웠습니다. 이를 MiningComponent로 분리함으로써 기능의 경계를 명확히 하고 HUD 연동 작업이나 애니메이션 수정 시에도 해당 컴포넌트만 집중적으로 관리할 수 있어 구조 파악과 기능 수정이 훨씬 쉬워졌습니다.

- **코드 흐름의 중요성 체감**  
여러 기능이 한 클래스에 뒤섞여 있을 때 디버깅이나 추가 구현이 어려웠습니다. 각 기능을 역할 단위로 나누자, 타일 배치나 자원 채굴 시 문제가 생겼을 때 빠르게 원인을 파악하고 수정할 수 있었고 전체 코드에 대한 이해도도 높아졌습니다.

- **협업을 위한 코드 작성 습관 개선**  
Sourcetree를 통한 버전 관리와 함께, Push 시 작업 내용을 팀원에게 요약 보고하는 규칙을 운영했습니다. 또한 함수 단위 주석과 의미 있는 변수명 작성에 집중하여 다른 팀원이 제 코드를 읽고 쉽게 이해하고 이어서 작업할 수 있도록 신경 썼습니다. 그 결과, 팀원들로부터 "읽기 쉬운 코드였다"는 피드백을 받았습니다.

- **소통과 질문의 중요성 인식**  
팀원 간 구현 방식이 달라 생기는 혼선을 줄이기 위해 적극적으로 질문하고 코드 흐름을 설명하는 주석을 상세히 남겼습니다. 그 결과 팀원들이 빠르게 이해하고 응답해줘 작업 속도가 향상되었고 프로젝트 마감 4일 전까지 핵심 기능을 완성할 수 있었습니다.
 : 개인 프로젝트와 달리, 협업 프로젝트에서는 "질문을 빠르게 주고받는 것"이 전체 진행 속도에 직접적인 영향을 준다는 것을 느꼈습니다. 혼자 고민하지 않고 소통하는 습관이 더욱 중요하다는 것을 배웠습니다.
---

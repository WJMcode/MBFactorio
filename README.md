...
**구조 설계 경험** : 기능 구현에 집중하던 초기에 유지보수가 어려워졌고, 역할 분리와 구조화를 통해 가독성과 디버깅 효율이 크게 향상되는 경험을 하였습니다. 이를 통해, 기능뿐만 아니라 구조 설계도 게임 개발의 핵심이라는 점을 배웠습니다.
**컴포넌트 기반 리팩토링 경험** : 코드 흐름을 명확하게 나누고, 각 기능을 재사용 가능한 컴포넌트로 분리함으로써 팀원 간 이해도와 작업 효율이 높아졌습니다. 특히, 채굴 시스템을 컴포넌트화한 것이 유지보수에 큰 도움이 되었습니다.
**효율적인 협업 방식 정립** : Sourcetree를 활용하여 커밋 시 작업 내용과 다음 작업을 간단히 공유하는 규칙을 팀 내에 적용하였습니다. 프로젝트가 급하게 시작되었음에도 이 방식 덕분에 4일 일찍 핵심 기능을 완성할 수 있었습니다.
**가독성 중심의 개발 습관 형성** : 팀원들도 제 코드를 참고하여 기능을 연동해야 했기 때문에, 주석 작성과 함수/변수의 명확한 명명에 더 신경 쓰게 되었습니다. 그 결과, 팀원들로부터 "읽기 쉬운 코드였다"는 피드백을 받았습니다.
**소통과 질문의 중요성 인식** : 개인 프로젝트와 달리, 팀 프로젝트에서는 "질문을 빠르게 주고받는 것"이 전체 진행 속도에 직접적인 영향을 준다는 것을 느꼈습니다. 혼자 고민하지 않고 소통하는 습관이 더욱 중요하다는 것을 배웠습니다.

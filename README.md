...
- **주요 함수**  
	복잡한 채굴 흐름을 **함수 단위로 명확히 분리**하였습니다.  
...
### 1. TileGridManager
#### 타일 랜덤 배치 및 자원 타입/머티리얼 자동 할당

  - **개요**  
플레이어 주변에 **Ground**, **Resource**, **Structures** 타일을 **확률적으로 배치**하여 매번 새로운 환경을 자동 생성하는 시스템입니다.  
각 타일의 속성은 미리 정의된 `DataAsset`을 바탕으로 자동 설정되어, **확장성과 유지보수성**이 뛰어납니다.

- **핵심 로직**  
	- `SpawnTiles` 함수에서 **타일 클래스를 받아**, **지정된 확률과 조건**에 따라 타일을 **배치**합니다.  
	- **자원 타일**의 경우 `DataAsset`에 정의된 **자원 타입 및 머티리얼 세트 중 하나를 무작위로 선택**하여 적용합니다.  
	- **땅 타일**은 **머티리얼만 무작위로 적용**되며, **구조물 타일**은 **미리 지정된 값으로 설정**됩니다.

- 플레이어 위치를 기준으로 2차원 그리드의 중심을 산출합니다.
- 각 셀마다 **생성 확률**(SpawnProbability)을 적용해, 난수로 타일 생성 여부를 결정합니다.
- 생성 위치, 크기, (필요 시) 랜덤 회전을 계산해 타일을 생성합니다.
- 생성된 타일의 타입에 따라 아래와 같이 속성을 자동 설정합니다:
    - **GroundTile:** 랜덤 머티리얼 적용
    - **ResourceTile:** DataAsset에서 랜덤으로 자원 타입·머티리얼 세트 선택 및 적용
    - **StructuresTile:** DataAsset에 정의된 타입, 머티리얼, 크기 적용
- 생성 실패 또는 잘못된 클래스 사용 시 경고 로그를 출력하여 디버깅이 쉽도록 구성했습니다.

- **설계 장점**
	- 게임 세계를 시작할 때마다 다른 자원 구성을 제공해 리플레이 가치를 높입니다.
	- `DataAsset` 기반으로 타입/머티리얼 데이터를 분리해 유지보수가 용이합니다.
 	- 각 타일 클래스별 분기 처리로 코드 가독성과 확장성이 좋습니다.
...
> **매번 다른 땅과 자원이 배치되어 새 게임마다 새로운 재미를 제공합니다.**

...
- **설계 장점**
	- **TryStartMining**
  - 입력 시간 누적을 통해 **의도치 않은 짧은 클릭(광클)을 방지**합니다.
  - 조건 충족 시에만 실제 채굴 로직이 실행되어 **불필요한 연산을 최소화**합니다.

	- **StartMining**
  - **프로그래스 HUD 출력 → 인벤토리 반영 → 애니메이션 출력**의 흐름이 명확하게 나뉘어 있어,  
    **기능별 확장 및 디버깅이 용이**합니다.
  - **자원 타입에 따라 인벤토리 처리가 자동화**되어 있어, 새로운 자원 추가 시 유지보수가 쉽습니다.

	- **StopMining**
  - 입력 해제 또는 조건 미충족 시 즉시 호출되어,  
    **채굴 관련 상태가 명확하게 초기화**됩니다.
  - HUD 및 애니메이션 중지를 포함하여 **시각적 피드백을 즉각적으로 관리**할 수 있습니다.

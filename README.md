...
- **타일 클래스 설계** : 공통 기능과 메시를 관리하는 `Tile` 클래스를 설계하고  
**Ground, Resource, Structures** 등 다양한 하위 클래스를 구성하였습니다.
- **타일 데이터 구조화** : C++ 구조체(`TileStructs`)를 통해 **타일 타입과 머티리얼을 정의**하였으며,
언리얼 에디터의 DataAsset(`TileDataAsset`)을 사용해 실제 데이터를 관리하고 있습니다.  
	- `TileStructs`에는 **자원 타입별 머티리얼 배열**과 **구조물 타입 및 머티리얼**이 포함되어 있습니다.
	- `TileDataAsset`은 **타일 크기, 타일별 클래스, 머티리얼 세트** 등을 담아 타일 생성에 활용됩니다.

...
- **채굴 컴포넌트**  
자원(Resource) 타일과의 상호작용, 채굴 진행도 UI, 인벤토리 반영까지 연결되는 **채굴 흐름을 컴포넌트화**하여 개발하였습니다.
...
- **개요**  
`MiningComponent`는 플레이어가 자원(Resource) 타일과 오버랩된 상태에서 **우클릭(Hold) 입력으로 자원을 채굴**할 수 있도록 구현된 재사용 가능한 컴포넌트입니다.  
...
- **협업 및 소통 역량** : 팀 프로젝트 중 이해가 어려운 기능에 대해 주저하지 않고 질문하며, 빠르게 문제를 해결하고 작업 효율을 높이는 경험을 했습니다. 이 과정을 통해 질문을 두려워하지 않는 태도가 협업에서 매우 중요하다는 것을 배웠습니다.
